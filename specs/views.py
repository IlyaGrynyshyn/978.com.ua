...
class CreateNewCategory(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            new_category.name = form.cleaned_data['title']
            new_category.save()
        return HttpResponseRedirect('/product-specs/')
...
class FeatureChoiceView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        feature_key_qs = CategoryFeature.objects.filter(
            category__slug=(request.GET.get('category_slug'))
        )
        res_string = ""
        ...
        return JsonResponse({"result": html_select, "value": (request.GET.get('category_slug'))})


class CreateFeatureView(View):

    def get(self, request, *args, **kwargs):
        category_id = request.GET.get('category_slug')
...
class AttachNewFeatureToProduct(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        for item in category_features:
            res_string += option.format(value=item.category.slug, option_name=item.feature_name)
        html_select = html_select.format(result=res_string)
